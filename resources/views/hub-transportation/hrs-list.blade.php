                    ...
                    <div class="d-flex justify-content-between align-items-center px-3 pb-3" style="gap: 1rem;">
                        ...
                        <div class="d-flex justify-content-center align-items-center"
                            ...
                            <input class="form-control form-control-sm"
                                style="width: 100%; height: 38px; padding-left: 2rem" id="myInput" type="text"
                                placeholder="search..." />
                        </div>
                    </div>
                    @csrf
                    <table id="unverified-table" class="table table-hover" style="width:100%">
                        ...
                        <tbody>
                            ...
                            foreach ($consignments as $key => $consignment) {
                                ...
                                $route_branch_id = explode(',',$consignment->route_branch_id);
                                $current_branch = end($route_branch_id);
                            ...
                            <tr>
                                <td style="text-align: center">
                                    <input type="checkbox" name="checked_consign[]" class="chkBoxClass ddd"
                                        value="{{$consignment->id}}" data-trp=""
                                        data-nexthub="{{ Helper::getNextHubId($consignment->id)}}" data-vctype=""
                                        style="width: 18px; height:18px;">
                                </td>
                                <td>{{ $consignment->id ?? "-" }}</td>
                                <td>{{ $consignment->consignment_date}}</td>
                                <td>{{ Helper::getNextHub($consignment->id)}}</td>
                                <td>{{ $consignment->ToBranch->name ?? "-"}}</td>
                                <td title="{{ $consignment->ConsignerDetail->nick_name}}">
                                    <p class="textWrap">{{ $consignment->ConsignerDetail->nick_name}}</p>
                                </td>
                                <td title="{{ $consignment->ConsigneeDetail->nick_name}}">
                                    <p class="textWrap">{{ $consignment->ConsigneeDetail->nick_name}}</p>
                                </td>
                                <td style="text-transform: uppercase">
                                    {{ $consignment->ConsigneeDetail->city ?? "-" }} -
                                    {{ $consignment->ConsigneeDetail->postal_code ?? "-" }}<br />
...
$('#create_hrs').click(function() {
        ...
        var checkboxes = document.querySelectorAll('input[type="checkbox"]');
            ...
            for (var i = 0; i < checkboxes.length; i++) {
                ...
                if (checkbox.checked) {
                var nextHub = checkbox.dataset.nexthub;

                // Check if the next hub is already selected
                if (selectedNextHub === null) {
                    selectedNextHub = nextHub;
                } else if (selectedNextHub !== nextHub) {
                    // Prompt the user to choose the same next hub
                    swal('error',"Please choose the same hub for HRS.",'error');
                    return false;
                }
                }
            }
            ...
            $(':checkbox[name="checked_consign[]"]:checked').each(function() {
                ...
                checkboxes.push($(this).data('nexthub'));
            });
