            ...
            <div class="widget-content widget-content-area br-6">
                ...
                <div class="col-lg-12 col-12 layout-spacing">
                    <div class="statbox widget box box-shadow">
                        <form class="general_form" method="POST" action="{{url($prefix.'/clients/update-client')}}"
                            id="updateclient">
                                    ...
                                    <tr class="rowcls">
                                        ...
                                        <td>
                                            <select class="form-control location_id" name="data[{{$i}}][location_id]">
                                                ...
                                                ?>
                                                <option value="{{ $key }}"
                                                    {{$regclientdata->location_id == $key ? 'selected' : ''}}>
                                        ...
                                        <td>
                                            <select class="form-control" name="data[{{$i}}][is_multiple_invoice]">
                                                <option value="">Select..</option>
                                                <option value="1"
                                                    {{$regclientdata->is_multiple_invoice == '1' ? 'selected' : ''}}>Per
                                                    invoice-Item wise</option>
                                                <option value="2"
                                                    {{$regclientdata->is_multiple_invoice == '2' ? 'selected' : ''}}>
                                                    Multiple Invoice-Item wise</option>
                                                <option value="3"
                                                    {{$regclientdata->is_multiple_invoice == '3' ? 'selected' : ''}}>per
                                                    invoice-Without Item</option>
                                                <option value="4"
                                                    {{$regclientdata->is_multiple_invoice == '4' ? 'selected' : ''}}>LR
                                            ...
                                            <div class="check-box d-flex">
                                                <div class="checkbox radio">
                                                    <label class="check-label">Yes
                                                        <input type="radio" value='1' name="data[{{$i}}][is_prs_pickup]"
                                                            {{ ($regclientdata->is_prs_pickup=="1")? "checked" : "" }}>
                                                        <span class="checkmark"></span>
                                                    </label>
                                                </div>
                                                <div class="checkbox radio">
                                                    <label class="check-label">No
                                                        <input type="radio" name="data[{{$i}}][is_prs_pickup]" value='0'
                                                            {{ ($regclientdata->is_prs_pickup=="0")? "checked" : "" }}>
                                                        <span class="checkmark"></span>
                                                    </label>
                                                </div>
                                            </div>
...
function addrow() {
    ...
    rows += '</td>';
    rows += '<td>';
    rows += '<select class="form-control" name="data[' + i +
