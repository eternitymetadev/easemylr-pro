...
@section('content')
    ...
    <div class="layout-px-spacing">
        <div class="page-header layout-spacing">
            <h2 class="pageHeading">Edit Vendor</h2>
        </div>
        ...
        <div class="widget-content br-6">
            <div class="col-lg-12 col-12 layout-spacing">
                <form id="update_vendor" method="POST" action="{{url($prefix.'/vendor/update-vendor')}}">
                    @csrf
                    <input type="hidden" class="form-control" name="vendor_id" value="{{$getvendor->id}}"/>
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control  my-select2" id="vendor_type" name="vendor_type"
                                ...
                                <option
                                    value="Individual" {{$getvendor->vendor_type == 'Individual' ? 'selected' : ''}}>
                                    Individual
                                </option>
                                <option
                                    value="Proprietorship" {{$getvendor->vendor_type == 'Proprietorship' ? 'selected' : ''}}>
                                    Proprietorship
                                </option>
                                <option
                                    value="Company" {{$getvendor->vendor_type == 'Company' ? 'selected' : ''}}>
                                    Company
                                </option>
                                <option
                                    value="Firm" {{$getvendor->vendor_type == 'Firm' ? 'selected' : ''}}>
                                    Firm
                                </option>
                                <option value="HUF" {{$getvendor->vendor_type == 'HUF' ? 'selected' : ''}}>
                                    HUF
                                </option>
                            </select>
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control  my-select2" id="branch_id" name="branch_id"
                                ...
                                @foreach($branchs as $branch)
                                    <option
                                        value="{{$branch->id}}"{{ $branch->id == $getvendor->branch_id ? 'selected' : ''}}>{{$branch->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" name="transporter_name" placeholder=""
                                   value="{{old('$otherdetails->transporter_name',isset($otherdetails->transporter_name)?$otherdetails->transporter_name:'')}}">
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control  my-select2" id="driver_id" name="driver_id"
                                ...
                                @foreach($drivers as $driver)
                                    <option
                                        value="{{$driver->id}}" {{ $driver->id == $getvendor->driver_id ? 'selected' : ''}}>{{$driver->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control" name="pan" placeholder=""
                                   value="{{old('pan',isset($getvendor->pan)?$getvendor->pan:'')}}">
                        </div>
                        <div class="form-group col-md-3">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control  my-select2" id="gst_register" name="gst_register"
                                    tabindex="-1">
                                <option
                                    value="Unregistered" {{$getvendor->gst_register == 'Unregistered' ? 'selected' : ''}}>
                                    Unregistered
                                </option>
                                <option
                                    value="Registered" {{$getvendor->gst_register == 'Registered' ? 'selected' : ''}}>
                                    Registered
                                </option>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control" id="gst_no" name="gst_no" placeholder=""
                                   value="{{old('gst_no',isset($getvendor->gst_no)?$getvendor->gst_no:'')}}"
                                   disabled>
                        </div>
                            ...
                            <div class="radioInputBox">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input no_decl" type="radio"
                                           name="decalaration_available" id="cds"
                                           value="1" {{$getvendor->declaration_available == '1' ? 'checked' : ''}}>
                                    <label class="form-check-label" for="inlineRadio1">Yes</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input no_decl" type="radio"
                                           name="decalaration_available" id=""
                                           value="2" {{$getvendor->declaration_available == '2' ? 'checked' : ''}}>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control" id="tds_rate" name="tds_rate"
                                   placeholder=""
                                   value="{{old('tds_rate',isset($getvendor->tds_rate)?$getvendor->tds_rate:'')}}"
                                   readonly>
                        </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" name="contact_person_number"
                                   placeholder=""
                                   value="{{old('$otherdetails->contact_person_number',isset($otherdetails->contact_person_number)?$otherdetails->contact_person_number:'')}}">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="exampleFormControlSelect1">Contact Email</label>
                            <input type="email" class="form-control" id="email" name="email" placeholder=""
                                   value="{{old('email',isset($getvendor->email)?$getvendor->email:'')}}">
                        </div>
                    </div>

                    <div class="form-row">
                        <h6 class="col-12">NEFT details</h6>
                        <?php $bankdetails = json_decode(old('bank_details', isset($getvendor->bank_details) ? $getvendor->bank_details : ''));?>
                        <div class="form-group col-md-4">
                            <label for="exampleFormControlInput2">Account Holder Name<span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="acc_holder_name" placeholder=""
                                   value="{{old('$bankdetails->acc_holder_name',isset($bankdetails->acc_holder_name)?$bankdetails->acc_holder_name:'')}}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="exampleFormControlInput2">Bank Name</label>
                            <input type="text" class="form-control" name="bank_name" placeholder=""
                                   value="{{old('$bankdetails->bank_name',isset($bankdetails->bank_name)?$bankdetails->bank_name:'')}}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="exampleFormControlInput2">Branch Name</label>
                            <input type="text" class="form-control" name="branch_name" placeholder=""
                                   value="{{old('$bankdetails->branch_name',isset($bankdetails->branch_name)?$bankdetails->branch_name:'')}}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="exampleFormControlSelect1">Account No.<span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="account_no" name="account_no"
                                   placeholder=""
                                   value="{{old('$bankdetails->account_no',isset($bankdetails->account_no)?$bankdetails->account_no:'')}}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="exampleFormControlInput2">Ifsc Code<span
                                    class="text-danger">*</span><span class="ml-1" style="color:red;"
                                                                      id="ifsc-error"></span></label>
                            <input type="text" class="form-control" id="" name="ifsc_code" placeholder=""
                                   value="{{old('$bankdetails->ifsc_code',isset($bankdetails->ifsc_code)?$bankdetails->ifsc_code:'')}}">
                        </div>
                        <div class="form-group col-md-4">
                    ...
                    <button type="submit" style="width: 120px; float: right;" class="mt-4 mb-4 btn btn-primary">
                        Update
                    </button>
...
@section('js')
    <script>
        ...
        $('input[name="ifsc_code"]').blur(function () {
            var ifsc = $(this).val();
