        ...
        $callback = function() use($payment_lists, $columns) {
             ...
             foreach ($payment_lists as $key => $payment_list) {
                ...
                if ($histrycount > 1) {
                    ...
                    $current_paid_amount2 = @$trans_id[1]->current_paid_amt;
                } else {
                ...
                $current_paid_amount1 = $trans_id[0]->current_paid_amt;
                if ($payment_list->payment_type == 'Balance') {
                ...
                $row['date'] = $date;
                $row['client'] = @$regn;
                $row['depot'] = @$payment_list->PaymentRequest[0]->Branch->nick_name;
                $row['station'] = @$city;
                $row['drs_no'] = $newDrs;
                $row['lr_no'] = @$multilr;
                $row['lr_inv'] = @$lr_itm;
                $row['type_of_vehicle'] = @$group_vehicle_type;
                $row['no_of_carton'] = @$ttqty;
                $row['net_wt'] = @$groupwt;
                $row['gross_wt'] = @$groupgross;
                $row['truck_no'] = @$group_vehicle;
                $row['vendor_name'] = @$payment_list->PaymentRequest[0]->VendorDetails->name;
                $row['vendor_type'] = @$payment_list->PaymentRequest[0]->VendorDetails->vendor_type;
                $row['declaration'] = @$decl;
                $row['tds_rate'] = @$payment_list->PaymentRequest[0]->VendorDetails->tds_rate;
                $row['bank_name'] = @$bankdetails->bank_name;
                $row['account_no'] = @$bankdetails->account_no;
                $row['ifsc_code'] = @$bankdetails->ifsc_code;
                $row['vendor_pan'] = @$payment_list->PaymentRequest[0]->VendorDetails->pan;
                $row['purchase_freight'] = @$payment_list->PaymentRequest[0]->total_amount;
                $row['paid_amt'] = @$paid_amt;
                $row['tds_amt'] = @$tds_amt;
                $row['balance_due'] = @$balance_due;
                $row['current_advance'] = @$current_paid_amount1;
                $row['advance'] = @$advan;
                $row['tds_deduct1'] = @$tds_cut;
                $row['payment_date'] = @$payment_date_1;
                $row['ref_no'] = @$trans_id[0]->bank_refrence_no;
                $row['current_balance'] = @$current_paid_amount2;
                $row['balance_amt'] = @$curr_paid_amt;
                $row['tds_deduct2'] = @$tds_cut1;
                $row['payment_date_2'] = @$payment_date_2;
                $row['ref_no_2'] = @$ref_no_2;
